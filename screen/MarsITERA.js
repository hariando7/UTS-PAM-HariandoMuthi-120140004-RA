...
const MarsITERA = ({ navigation }) => {
    const [sound, setSound] = React.useState();

    async function playSound() {
        console.log("Loading Sound");
        const { sound } = await Audio.Sound.createAsync(
            require("../assets/MarsITERA.mp3")
        );
        setSound(sound);
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                <View style={styles.top}>
                    ...
                    >
                        <AntDesign name="arrowleft" size={24} color="#3a1cff" />
                    </TouchableOpacity>
                ...
                <Text style={styles.heading2}>Hariando Muthi</Text>
            </View>
            <View style={styles.detail}>
                <Text style={styles.memutar}>Pemutar Musik Mars ITERA</Text>
                <Image
                    style={{ width: 300, height: 300, borderRadius: 10, marginTop: 25 }}
                    source={require('../assets/iteraGerbang.png')}
                />
                <Text style={styles.judulLagu}>Mars ITERA</Text>
                <Text style={styles.pencipta}>Institut Teknologi Sumatera</Text>
            </View>
